...
that represents a population of users \sm{, but there might not be a unique truth~\citep{aroyo2015truth}}.
...
\sm{This argument is far from being conclusive but it confirms that
  the ten workers that we used in our experiment, and the associated
  cost, can be reduced. 
  Therefore, from the about $\$0.4$ for each document figure mentioned
  in Section~\ref{sec:crowd-judging} we can estimate that only about
  $\$0.3$  for each document are really needed, if not less since more
  efficient methods could be devised.}
...
\sm{Let us wrap up an overall answer to  RQ\ref{item:rq4}. As we anticipated, using ME with crowdsourcing could potentially raise several concerns, including: subjectiveness of the scale, normalization issues, lack of proper training, potential internal and/or external disagreement, and overall cost in terms of the number of redundant workers needed.}
\sm{On the basis of the results presented so far we can conclude that, at least with our setting and our quality checks in place (see items (a)--(d) in Section~\ref{sec:quality-checks}), we can be reassured that magnitude estimation relevance scores gathered by means of crowdsourcing are reliable. Also, the cost is competitive, as in our experiment it is in the same range of the cost of collecting ordinal scale judgments, and there are clues that it can be lowered by collecting fewer judgments.}
